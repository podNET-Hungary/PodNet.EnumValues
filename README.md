...
A simple and powerful C# code generator that lets you define string values for your enums to implement 🔥🔥🔥 lookups.
...
1. You also get an analyzer and code fix if you miss a value:
  ![Screenshot from Visual Studio showing code fix in action](../../images/demo-missing-member-code-fix.png)
   You can also configure this warning (or any other generated warnings) to appear as an error, or silence it individually or globally (although the latter is not recommended).
   > Note that unfortunately, it's not currently possible to offer a "rename" functionality when generating code for string literal values, so you'll have to inspect and overwrite the generated name yourself (which defaults to the name of the enum member).
...
> For reference and your viewing pleasure, we'll supply the code here as well:
>
> ```csharp
> // <auto-generated />
>#nullable enable
...
> ```
>
> Note how the generated code now also accounts for the `Unknown` value throwing a `MissingValueException`, and this is also reflected in the generated documentation.
...
> **NOTE**: it's a requirement for the descendant to have its first constructor argument be the `string` value (but it can also be a referenced constant, not just inline). No other requirements, so you can supply more arguments if it suits your needs.
...
> }
> ```
> Note that C# is awesome.

Then, apply your own derived `ValueAttribute` as type parameter to `[Values<T>]`:
