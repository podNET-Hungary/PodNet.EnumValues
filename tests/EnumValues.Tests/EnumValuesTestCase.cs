...
public class EnumValuesTestCase<T>(List<DiagnosticDescriptor>? expectedDescriptorsForGenerator = null, bool ignoreSources = false) : EmbeddedTestCase<EnumValuesGenerator, T>
{
    ...
    public override CSharpCompilation CreateCompilation(ImmutableArray<SyntaxTree> syntaxTrees, ImmutableArray<AdditionalText> additionalFiles)
        => base.CreateCompilation(syntaxTrees.Add(CSharpSyntaxTree.ParseText("global using PodNet.EnumValues;")), additionalFiles);
        ...
        where TCodeFix : CodeFixProvider, new()
    {
        ...
        public override ScriptOptions ConfigureScriptOptions(ScriptOptions options)
            ...
            [TestMethod]
        public override async Task CodeFixWorksAsExpectedAsync()
        {
            await base.CodeFixWorksAsExpectedAsync();
        }
    }
}
